...
export const categoriesApi = {
  async list(params?: CategoryListParams): Promise<WooCommerceCategory[]> {
    ...
    return wooClient.request<WooCommerceCategory[]>(
      `/products/categories${query ? `?${query}` : ""}`
    );
  },

  async get(id: number): Promise<WooCommerceCategory> {
    return wooClient.request<WooCommerceCategory>(`/products/categories/${id}`);
  },

  async create(data: CreateCategoryDTO): Promise<WooCommerceCategory> {
    return wooClient.request<WooCommerceCategory>("/products/categories", {
      method: "POST",
  ...
  async batch(
    ...
    return wooClient.request("/products/categories/batch", {
      method: "POST",
